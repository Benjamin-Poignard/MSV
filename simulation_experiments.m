...
for oo = 1:Nsim
    ...
    p = 10; grid = (0.01:0.2:4);
    lambda = grid*sqrt(log(p*N^2)/T);
    
    % SCAD penalization
    [H_scad,b_scad,~,~,~,~] = SV_process(res,p,'scad','no-constant',lambda,folds);
    
    % MCP penalization
    [H_mcp,b_mcp,~,~,~,~] = SV_process(res,p,'mcp','no-constant',lambda,folds);
    
    % Non-penalized MSV
    [H_npen,b_nonpen,~,~,~,~] = SV_process(res,p,'nonpen','no-constant',lambda,folds);
    ...
    dist_npen = zeros(T,1);
    for tt = 1:T
        dist_scad(tt) = norm(H_true(:,:,tt)-H_scad(:,:,tt),'fro');
        dist_npen(tt) = norm(H_true(:,:,tt)-H_npen(:,:,tt),'fro');
        dist_mcp(tt) = norm(H_true(:,:,tt)-H_mcp(:,:,tt),'fro');
        distdcc(tt) = norm(H_true(:,:,tt)-Hdcc(:,:,tt),'fro');
        distccc(tt) = norm(H_true(:,:,tt)-Hccc(:,:,tt),'fro');
    end
...
for oo = 1:Nsim
    ...
    [H_scad,b_scad,~,~,~,~] = SV_process(res,p,'scad','no-constant',lambda,eta_p,len,3);
    
    [H_mcp,b_mcp,~,~,~,~] = SV_process(res,p,'mcp','no-constant',lambda,eta_p,len,3);
    
    [H_npen,b_nonpen,~,~,~,~] = SV_process(res,p,'nonpen','no-constant',lambda,eta_p,len,3);
