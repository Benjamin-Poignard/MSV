function [res,Sigma] = DGP_march50(T,N)
...
while cond
    ...
    Omega = rand(N,N); Omega = temp.*(Omega*Omega'/1900);
    for ii = 1:N
        Omega(ii,ii) = 0.08+0.09*rand(1);
    end
    
    temp = round(0.75*rand(N^2)); temp(temp==0)=-1;
    B_temp = temp.*tril((0.05+(0.2-0.05)*rand(N^2))/5500,0);
    B = B_temp + B_temp';
    
    for ii = 1:N^2
        B(ii,ii) = 0.015+(0.024-0.015)*rand(1);
     end
