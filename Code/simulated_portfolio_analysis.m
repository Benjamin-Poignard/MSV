...
[~,B,Sig_zeta,Sig_alpha,Gamma] = SV_process_estim_memo(res_in,p,'alasso','no-constant',lambda,folds);
...
% SCAD penalised MSV
lambda = 0.5*(0.1:0.1:1.5)*sqrt(log(p*N^2)/T);
[~,B,Sig_zeta,Sig_alpha,Gamma] = SV_process_estim_memo(res_in,p,'scad','no-constant',lambda,folds);
...
% MCP penalised MSV
lambda = 0.5*(0.1:0.1:1.5)*sqrt(log(p*N^2)/T);
[~,B,Sig_zeta,Sig_alpha,Gamma] = SV_process_estim_memo(res_in,p,'mcp','no-constant',lambda,folds);
...
% Non-penalised MSV
lambda = 0;
[~,B,Sig_zeta,Sig_alpha,Gamma] = SV_process_estim_memo(res_in,p,'nonpen','no-constant',lambda,folds);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%% Estimation of the GARCH based competitors %%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% GO-GARCH
gogarch_parameters = gogarch(res_in,1,1,2,'GOGARCH');

% scalar BEKK
% the scalar model requires the estimation of the "intercept" matrix, which
% that is N(N+1)/2 coefficients. This may take a long time: a variance
% targeting approach may be more suitable once N > 30, 40
[parameters_bekk,~,~,~,~,~,~]=scalar_bekk_mvgarch(res_in);

% scalar DCC
[parameters_dcc, Rt,H_in]=dcc_mvgarch(res_in);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Generation of the out-of-sample forecasts for the scalar BEKK %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
dim = N*(N+1)/2;
[~,~,Hbekk]=scalar_bekk_mvgarch_likelihood_oos(parameters_bekk,res_out,res_in,1,1,N,dim,length(res_out));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Generation of the out-of-sample forecasts for the Orthogonal GARCH %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
R = res_out; r_temp = R;
if ndims(r_temp)==3
    [k,~,~] = size(r_temp);
    ...
    temp = zeros(k,k,T);
    for t=1:T
        temp(:,:,t) = r_temp(t,:)'*r_temp(t,:);
    end
    r_temp = temp;
end
S = cov(res_in); [P,L] = eig(S); 
[~,~,Hog] = gogarch_likelihood(gogarch_parameters,r_temp,ones(N,1),ones(N,1),2*ones(N,1),P',L,false,false);

% DM-test: first, obtain the GMVP based portfolio weights
wdcc = zeros(N,T); wbekk = zeros(N,T); wgo = zeros(N,T);
...
e9 = zeros(T,1); e10 = zeros(T,1); e11 = zeros(T,1);
...
for t = 1:T
    ...
    e2(t) = wgo(:,t)'*R(t,:)';
    e3(t) = wbekk(:,t)'*R(t,:)';
    e4(t) = wmsv_unpen(:,t)'*R(t,:)';
    e5(t) = wmsv_al(:,t)'*R(t,:)';
    e6(t) = wmsv_scad(:,t)'*R(t,:)';
    e7(t) = wmsv_mcp(:,t)'*R(t,:)';
    e8(t) = wmsv_unpen_l(:,t)'*R(t,:)';
    e9(t) = wmsv_al_l(:,t)'*R(t,:)';
    e10(t) = wmsv_scad_l(:,t)'*R(t,:)';
    e11(t) = wmsv_mcp_l(:,t)'*R(t,:)';
end
e1 = e1.^2; e2 = e2.^2; e3 = e3.^2; e4 = e4.^2; e5 = e5.^2; e6 = e6.^2; e7 = e7.^2;
e8 = e8.^2; e9 = e9.^2; e10 = e10.^2; e11 = e11.^2;
E = [e1 e2 e3 e4 e5 e6 e7 e8 e9 e10 e11];

% Generate the table for the DM test results, where the columns are as:
% DCC &  O-GARCH & BEKK & MSV(10) & MSV-AL(10) & MSV_SCA(10) & MSV_MCP(10) & MSV(20) & MSV-AL(20) & MSV_SCA(20) & MSV_MCP(20)
DM2 = [];
for kk = 1:size(E,2)
    DM = [];
    for ii = 1:size(E,2)
